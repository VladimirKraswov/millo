...
The program scene always treats the controller work position as the source of
truth for the tool tip. X, Y and Z therefore move together with live GRBL
status; the dashed projection in isometric view shows where the tip sits over
the program plane. Rendering smooths the gap between status samples but never
predicts motion or changes the machine snapshot.

Jobs generated by PCB and surfacing workflows carry the selected library tool
for each `T` number. The scene shows its approximate cutter, tip, shank, flute
count and included angle, and switches the model at a host-managed tool-change
barrier. The HUD names the selected tool and reports selected, rotating, held,
changing, or removed state. Rotation means only that a physical processing run
is active; it is not evidence that a manually controlled spindle is actually
powered. Motion check deliberately hides the cutter body and retains the live
position marker.
