...
| [Candle #464](https://github.com/Denvi/Candle/issues/464): request to run from an arbitrary selected line | Re-entering midway with the wrong modal/position state can crash a tool | Raw send-from-line is deliberately absent; restart requires a future typed checkpoint, modal reconstruction, safe approach, and new authorization | Capability is fail-closed today |
