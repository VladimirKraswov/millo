...
`unload(id)` немедленно закрывает capabilities и убирает contributions, затем
ожидает `deactivate`. Пока он не завершился, загрузка того же ID отклоняется;
повторный `unload` ожидает ту же операцию. Сохранённый disposable старого
contribution не удалит новый contribution после перезагрузки.
Циклические `replaces` в одном slot отклоняются до изменения реестра, чтобы
взаимные замены не скрыли все рабочие панели.

Подписки `machine.read` и `tools.read` принадлежат activation scope. Ошибка
одного callback изолируется; после выгрузки даже поздний callback источника
не доставляется плагину. `current()` возвращает неизменяемый cached snapshot.
У tools snapshot native revision монотонна: поздний ответ не откатывает UI.

Генерация/parse выполняются в ограниченном native pool, отдельно от actor.
При занятости запрос может завершиться ошибкой: сохраняйте введённые настройки,
показывайте ошибку рядом с действием и позволяйте повторить его. Закрытие окна
не останавливает уже начавшийся CPU worker. После `await` проверяйте, что проект
не изменился; результат прежнего проекта нельзя автоматически открывать как новый.
`jobs.create` не выдаёт право запускать программу: `jobs.open` загружает задание,
а Check/подтверждение/Start остаются самостоятельным операторским действием.

### Большие задания и ось A

Полный текст задания находится в `GeneratedJob.source`. Не восстанавливайте
G-code из `program.lines` или из preview: загруженный native-документ может
содержать только первую страницу строк и обзорную геометрию. Статистику всего
задания берите из `program.summary`. Постраничное чтение и точная геометрия
выбранной строки относятся к host `ProgramGateway`, не к прямым Tauri-вызовам
из плагина. Существующий `jobs.open(job)` передаёт задание обычному workspace.

Наличие `machine.current().machine.workPosition.a` само по себе не выдаёт
права выполнять четырёхосевой код. Градусы A, совместимость firmware, текущая
привязка и Check проверяются command actor. Существующие CAM-генераторы не
стали четырёхосевыми от появления поддержки sender. Ограничения больших файлов,
карты высот и преобразований A описаны в
[LARGE_PROGRAMS_AND_ROTARY](LARGE_PROGRAMS_AND_ROTARY.md).
