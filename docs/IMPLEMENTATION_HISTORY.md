# История реализации прототипа

Снимок прежнего README на 2026-09-05. Это этапы разработки, не текущие
гарантии продукта. Актуальные границы: [PRODUCT.md](PRODUCT.md).
...
[`docs/ALPHA_RELEASE.md`](ALPHA_RELEASE.md). The website and its moderated
...
[Virtual controller](VIRTUAL_CONTROLLER.md).
...
write. See [Machine control lifecycle](MACHINE_CONTROL.md) and
[ADR 0057](decisions/0057-actor-owned-homing-and-continuous-jog.md).
...
[Operator console](OPERATOR_CONSOLE.md) and
[ADR 0058](decisions/0058-read-only-operator-console.md).
...
[heightmap visualization](HEIGHTMAP_VISUALIZATION.md) and
[ADR 0052](decisions/0052-workpiece-heightmap-session.md).
The implementation choices and follow-up ideas taken from GRBL, Candle, CNCjs,
bCNC, and UGS are recorded in
[Probing and heightmap comparison](PROBING_COMPARISON.md).
...
revalidate the repeat run. See [ADR 0047](decisions/0047-return-to-work-zero.md).
...
[ADR 0049](decisions/0049-safe-program-editor.md).
...
[ADR 0048](decisions/0048-safe-selected-line-start.md).
...
[Millo sender compared with Candle](CANDLE_SENDER_COMPARISON.md). The
current command matrix and physical evidence are in
[GRBL execution audit](GRBL_EXECUTION_AUDIT.md). Issue-driven reliability
work and explicit limitations are recorded in
[Sender hardening](SENDER_HARDENING.md).
...
[External plugins and macros](EXTERNAL_PLUGINS.md) and
[Plugin development guide](PLUGIN_DEVELOPMENT.md) describe the two plugin
levels, stable SDK imports, capability lifecycle, package schema and tests. See
[ADR 0050](decisions/0050-sandboxed-external-plugins.md).
...
recorded in [ADR 0039](decisions/0039-progressive-operator-shell.md).
...
[ADR 0043](decisions/0043-job-centered-operator-workflow.md).
...
[ADR 0044](decisions/0044-sparse-grbl-status-reconciliation.md).
...
See [Image jobs](IMAGE_JOBS.md) and
[ADR 0045](decisions/0045-generated-jobs-capability.md).
...
starts the spindle or reaches the sender. See [Tool library](TOOL_LIBRARY.md),
[Surfacing](SURFACING.md), and
[ADR 0046](decisions/0046-core-tool-library-and-surfacing-plugin.md).
...
hardware procedures are documented in [Testing](TESTING.md); the confirmed
...
remaining release boundaries are recorded in [`docs/CODE_AUDIT.md`](CODE_AUDIT.md).

See [Architecture](ARCHITECTURE.md), the decisions for the
[modular core](decisions/0001-modular-core.md) and
[controller lifecycle](decisions/0002-controller-lifecycle.md), plus the
[project naming decision](decisions/0003-project-name-millo.md) and
[native serial boundary](decisions/0004-native-serial-transport.md), and
[command arbiter](decisions/0005-command-arbiter-device-inspector.md), plus
[hardware readiness](decisions/0006-hardware-readiness.md) and
[realtime safety controls](decisions/0007-realtime-safety-controls.md), then
the [guarded step jog](decisions/0008-guarded-step-jog.md),
[actor-owned homing and continuous jog](decisions/0057-actor-owned-homing-and-continuous-jog.md),
[machine-scaled Motion Deck](decisions/0040-machine-scaled-motion-deck.md), and
[verified unhomed configuration](decisions/0009-unhomed-controller-configuration.md),
then the [extension host boundary](decisions/0010-extension-host-boundaries.md)
and [versioned plugin manifest](decisions/0011-versioned-plugin-manifest.md),
followed by the
[read-only machine capability](decisions/0012-machine-read-capability.md)
and [PluginHost bootstrap](decisions/0013-plugin-host-bootstrap.md), then the
[guarded work-zero transaction](decisions/0014-guarded-work-zero.md) and
[G-code program boundary](decisions/0015-gcode-program-boundary.md), then
the [Mock-only bounded sender](decisions/0016-mock-dry-run-sender.md) and
[immutable line selection](decisions/0017-program-line-selection.md), then
the [serial real-run preflight](decisions/0018-real-run-preflight.md), and
the [persistent machine-profile boundary](decisions/0019-machine-profiles.md),
the [controller settings and identity boundary](decisions/0020-controller-settings-sync.md),
and the [first-cut authorization boundary](decisions/0021-first-cut-authorization.md).
The test-only sender promotion is recorded in
[ADR 0022](decisions/0022-serial-sender-fixtures.md).
The production file sender is recorded in
[ADR 0023](decisions/0023-authorized-file-program-run.md).
Its bounded receive-buffer streaming contract is recorded in
[ADR 0025](decisions/0025-grbl-rx-buffer-streaming.md).
Full modal arc parsing and conservative time estimation are recorded in
[ADR 0026](decisions/0026-modal-parser-and-time-estimation.md).
The verified GRBL Check-run lifecycle is recorded in
[ADR 0027](decisions/0027-grbl-check-run.md).
Responsive interleaved sender I/O is recorded in
[ADR 0028](decisions/0028-responsive-sender-io.md).
Host-managed tool change is recorded in
[ADR 0034](decisions/0034-host-managed-tool-change.md).
Certified Check evidence and the bounded run journal are recorded in
[ADR 0037](decisions/0037-certified-check-and-run-journal.md).
Crash-safe guided restart is recorded in
[ADR 0038](decisions/0038-guided-power-loss-recovery.md).
The progressive operator shell and compact confirmation mapping are recorded in
[ADR 0039](decisions/0039-progressive-operator-shell.md).
The job-centered run flow and contextual action priority are recorded in
[ADR 0043](decisions/0043-job-centered-operator-workflow.md). Safe
selected-line compilation is recorded in
[ADR 0048](decisions/0048-safe-selected-line-start.md), and the separate
draft/parse/apply editor boundary in
[ADR 0049](decisions/0049-safe-program-editor.md). The separation between
trusted React plugins and imported Rhai packages is recorded in
[ADR 0053](decisions/0053-two-tier-plugin-contract.md). The
external serial virtual-machine boundary is recorded in
[ADR 0055](decisions/0055-external-serial-virtual-controller.md). The
required verification workflow is recorded in [Testing](TESTING.md); the
known first-machine configuration is in [Hardware target](HARDWARE_TARGET.md).
