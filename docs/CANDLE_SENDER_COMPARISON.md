...
| Community scripts | QtScript plugins can call broad application/device/sender surfaces and construct Qt widgets | Versioned `.millo-plugin` packages declare buttons, modal fields and capabilities; bounded Rhai returns one validated action, grants are bound to the reviewed SHA-256, and generated source is reparsed. Optional `machine.commands` still requires global expert mode, confirmation and the same guarded actor | Keeps useful user macros and firmware-specific expert commands without allowing a plugin to own serial, race the sender, inject realtime bytes, or retain silent authority after an update | `millo-script` sandbox/store fixtures, actor console fixtures, external contribution UI tests, ADR 0050/0058 |
...
- Arbitrary start/end strings remain outside the sender plan. A separately
  persisted expert preference can admit one console/plugin GRBL line, but it is
  never a raw transport write: it is actor-serialized, Idle/Alarm-only, blocked
  during operations, audited, capability/confirmation-gated for plugins, and
  invalidates stale Check/authorization/coordinate evidence.
