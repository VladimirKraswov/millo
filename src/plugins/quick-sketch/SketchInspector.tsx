...
import { Copy, Trash2, Ruler, Settings2 } from "lucide-react";
...
export function SketchInspector({
  ...
  const [tab, setTab] = useState<"geometry" | "machining">("geometry");
  ...
  return (
    <section className="sketch-properties">
      ...
      <div
        className="sketch-inspector-tabs"
        role="tablist"
        aria-label="Свойства фигуры"
      >
        <button
          type="button"
          role="tab"
        ...
        <>
          <label className="sketch-select">
            ...
            >
              {Object.entries(operationLabels)
                .filter(
                  ([kind]) => kind !== "drill" || geometry.kind === "circle",
                )
                .map(([kind, label]) => (
          ...
          <label className="sketch-select">
            ...
            <select
              ...
              onChange={(e) =>
                operation(
                  toolSettings(tools.find((t) => t.id === e.target.value)),
                )
              }
            >
