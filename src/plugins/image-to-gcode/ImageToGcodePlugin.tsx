...
export function ImageToGcodePlugin({
  jobs,
  initialOpen = false,
}: ImageToGcodePluginProps) {
  ...
  useEffect(() => {
    ...
    const url = URL.createObjectURL(
      new Blob([generated.vectorSvg], { type: "image/svg+xml" }),
    );
  ...
  const chooseFile = (file?: File) => {
    ...
    const format =
      extension === "svg" ? "svg" : extension === "png" ? "png" : undefined;
    if (!format) {
  ...
  return (
    <>
      ...
      {open &&
        ...
        createPortal(
          <div
            className="machine-dialog-backdrop image-job-backdrop"
            role="presentation"
          >
            <DialogSurface
              ...
              <div className="image-job-body">
                ...
                >
                  ...
                  <div className="image-source-meta" aria-live="polite">
                    ...
                    <code>
                      {selected
                        ? `${selected.format.toUpperCase()} · ${formatBytes(selected.file.size)}`
                        : "SVG / PNG"}
                    </code>
                  </div>
                  {generated && (
                    <dl className="image-job-result">
                      <div>
                        <dt>Размер</dt>
                        <dd>
                          {generated.summary.widthMm.toFixed(1)} ×{" "}
                          {generated.summary.heightMm.toFixed(1)} mm
                        </dd>
                      </div>
                      <div>
                        <dt>Контуры</dt>
                        <dd>{generated.summary.pathCount}</dd>
                      </div>
                      <div>
                        <dt>Точки</dt>
                        <dd>{generated.summary.pointCount}</dd>
                      </div>
                      <div>
                        <dt>G-code</dt>
                        <dd>{generated.program.summary.lineCount} строк</dd>
                      </div>
                    </dl>
                  )}
                </section>
                ...
                <section
                  className="image-job-settings"
                  aria-label="Параметры гравировки"
                >
                  <div className="image-job-fields">
                    ...
                    <NumberField
                      ...
                      min={0.001}
                      onChange={(value) =>
                        updateNumber("engravingDepthMm", value)
                      }
                      step={0.05}
                      suffix="mm"
                      value={settings.engravingDepthMm}
                    />
                    <NumberField
                      label="Безопасный Z"
                      max={1000}
                    ...
                    <NumberField
                      ...
                      max={20000}
                      min={1}
                      onChange={(value) => updateNumber("feedMmPerMin", value)}
                      step={50}
                      suffix="mm/min"
                    ...
                    />
                    <NumberField
                      label="Точность кривых"
                      max={2}
                      min={0.005}
                      onChange={(value) =>
                        updateNumber("curveToleranceMm", value)
                      }
                      step={0.01}
                      suffix="mm"
                      value={settings.curveToleranceMm}
                    />
                  </div>
                  ...
                  {selected?.format === "png" && (
                    <details className="image-trace-settings">
                      <summary>
                        <span>Векторизация PNG</span>
                        <ChevronDown aria-hidden="true" size={14} />
                      </summary>
                      <div>
                        <label className="image-range-field">
                          <span>
                            <strong>Порог яркости</strong>
                            <code>{settings.rasterThresholdPercent}%</code>
                          </span>
                          <input
                            max="99"
                            min="1"
                        ...
                        <NumberField
                          label="Удалять шум до"
                          max={64}
                          min={1}
                          onChange={(value) =>
                            updateNumber("traceSpecklePx", value)
                          }
                          step={1}
                          suffix="px"
                          value={settings.traceSpecklePx}
                        />
                  ...
                  <div
                    className={`image-job-status${error ? " is-error" : generated ? " is-ready" : ""}`}
                    aria-live="polite"
                  >
                    {error ? (
                      error
                      ...
                      "Выберите изображение и проверьте физический размер."
                    )}
                  </div>
                </section>
              </div>
