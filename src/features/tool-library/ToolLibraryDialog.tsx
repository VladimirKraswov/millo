...
export function ToolLibraryDialog({
  ...
  const updateKind = (kind: ToolKind) =>
    setDraft((current) => ({
      ...current,
      kind,
      tipDiameterMm:
        ...
        <div className="tool-library-body">
          <aside className="tool-library-list">
            <div className="tool-library-list-actions">
              ...
              <button
                aria-label="Восстановить стандартные инструменты"
                onClick={() => void restore()}
                title="Вернуть отсутствующие пресеты"
                type="button"
              >
              ...
              {library.tools.map((tool) => (
                ...
                >
                  ...
                  <span>
                    ...
                    <small>
                      {toolKindLabels[tool.kind]} ·{" "}
                      {tool.tipDiameterMm !== undefined
                        ? `кончик ${tool.tipDiameterMm} mm`
                        : `Ø ${tool.diameterMm} mm`}
                    </small>
                  </span>
          ...
          <main className="tool-library-editor">
            <div className="tool-editor-heading">
              ...
              <div>
                <span>
                  {creating ? "Новый инструмент" : toolKindLabels[draft.kind]}
                </span>
            ...
            <div className="tool-editor-scroll">
              <section className="tool-form-section">
                ...
                <div className="tool-form-grid">
                  <TextField
                    label="Название"
                    onChange={(value) => mutateDraft("name", value)}
                    value={draft.name}
                    wide
                  />
                  <label className="tool-field">
                    ...
                    <select
                      onChange={(event) =>
                        updateKind(event.target.value as ToolKind)
                      }
                      value={draft.kind}
                    >
                      {toolKinds.map((kind) => (
                  ...
                  <NumberField
                    label={
                      draft.tipDiameterMm !== undefined
                        ? "Макс. диаметр"
                        : "Диаметр"
                    }
                    max={500}
                    min={0.01}
                    onChange={(value) => mutateDraft("diameterMm", value)}
                    step={0.001}
                    suffix="mm"
                    value={draft.diameterMm}
                  />
                  {(draft.kind === "vBit" || draft.kind === "engraving") && (
                    <NumberField
                      label="Диаметр кончика"
                      max={draft.diameterMm}
                      min={0.001}
                      onChange={(value) => mutateDraft("tipDiameterMm", value)}
                      step={0.01}
                      suffix="mm"
                      value={draft.tipDiameterMm ?? 0.1}
                    />
                  )}
                  <NumberField
                    label="Хвостовик"
                    max={100}
                    min={0.1}
                    onChange={(value) => mutateDraft("shankDiameterMm", value)}
                    step={0.001}
                    suffix="mm"
                    value={draft.shankDiameterMm}
                  />
                  <NumberField
                    label="Режущая длина"
                    max={1000}
                    min={0.1}
                    onChange={(value) => mutateDraft("cuttingLengthMm", value)}
                    step={0.1}
                    suffix="mm"
                    value={draft.cuttingLengthMm}
                  />
                  <NumberField
                    ...
                    min={1}
                    onChange={(value) =>
                      mutateDraft("fluteCount", Math.round(value))
                    }
                    step={1}
                    suffix="шт"
                    value={draft.fluteCount}
                  />
                  {(draft.kind === "vBit" || draft.kind === "engraving") && (
                    <OptionalNumberField
                      label="Угол"
                      ...
                      min={1}
                      onChange={(value) =>
                        mutateDraft("includedAngleDegrees", value)
                      }
                      placeholder="Неизвестен"
                      step={1}
                      suffix="°"
                      value={draft.includedAngleDegrees}
                    />
                  )}
                </div>
              </section>
              ...
              <section className="tool-form-section">
                ...
                <div className="tool-form-grid">
                  <NumberField
                    ...
                    max={100000}
                    min={1}
                    onChange={(value) => mutateDraft("feedMmPerMin", value)}
                    step={50}
                    suffix="mm/min"
                    value={draft.feedMmPerMin}
                  />
                  <NumberField
                    label="Подача Z"
                    max={50000}
                    min={1}
                    onChange={(value) => mutateDraft("plungeMmPerMin", value)}
                    step={25}
                    suffix="mm/min"
                    value={draft.plungeMmPerMin}
                  />
                  <NumberField
                    label="Шпиндель"
                    max={100000}
                    min={1000}
                    onChange={(value) =>
                      mutateDraft("spindleRpm", Math.round(value))
                    }
                    step={1000}
                    suffix="rpm"
                    value={draft.spindleRpm}
                  />
                  <NumberField
                    label="Съём за проход"
                    max={draft.cuttingLengthMm}
                    min={0.001}
                    onChange={(value) => mutateDraft("stepdownMm", value)}
                    step={0.05}
                    suffix="mm"
                    value={draft.stepdownMm}
                  />
                  <NumberField
                    label="Перекрытие шага"
                    max={95}
                    min={1}
                    onChange={(value) => mutateDraft("stepoverPercent", value)}
                    step={1}
                    suffix="% Ø"
                    value={draft.stepoverPercent}
                  />
                </div>
              </section>
              ...
              <section className="tool-form-section">
                <h3>Описание</h3>
                <label className="tool-field is-wide">
                  <textarea
                    maxLength={2000}
                    onChange={(event) =>
                      mutateDraft("description", event.target.value)
                    }
              ...
              {selected && !creating && (
                <ToolKnowledgePanel tool={{ ...selected, ...draft }} />
              )}
            </div>
            <footer>
              <div
                className={`tool-editor-status${status?.toLowerCase().includes("invalid") ? " is-error" : ""}`}
                aria-live="polite"
              >
                <BookOpen aria-hidden="true" size={14} />
                {status ??
                  "Режимы зависят от материала, станка и вылета инструмента."}
              </div>
              <div>
                {!creating && selected && (
                  <button
                    className={deleteConfirming ? "is-danger" : ""}
                    disabled={busy}
                    onClick={() => void remove()}
                    type="button"
                  >
                    <Trash2 aria-hidden="true" size={14} />
                    {deleteConfirming ? "Подтвердить" : "Удалить"}
                  </button>
                )}
                <button
