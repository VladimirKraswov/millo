...
pub fn generate_pcb_job(
    ...
    if request.settings.outline.enabled {
        ...
        let tab_width = request.settings.outline.tab_width_mm + tool.diameter_mm;
        let perimeter = closed_perimeter(&path.iter().map(|p| (p.x(), p.y())).collect::<Vec<_>>());
        if request.settings.outline.tab_count > 0
            && f64::from(request.settings.outline.tab_count) * tab_width >= perimeter * 0.75
        {
            return Err(PcbError::InvalidSetting(
                "outline tabs occupy too much of the contour",
            ));
        }
...
impl Emitter<'_> {
    fn use_tool(&mut self, tool: &CuttingTool, label: &str) {
        writeln!(self.source, "; {}", comment(label)).unwrap();
        ...
        for (start, end) in slots {
            ...
            if length <= 0.000_5 {
                for pass in 1..=pass_count {
                    ...
                    motions +=
                        self.drill_hits(std::iter::once(start), pass_depth, tool.plunge_mm_per_min);
                }
                continue;
            }
    ...
    fn cut_outline(
        ...
        let intervals = (0..tab_count)
            .map(|index| {
                let center = perimeter * (f64::from(index) + 0.5) / f64::from(tab_count);
                (center - tab_width_mm / 2.0, center + tab_width_mm / 2.0)
            })
            .collect::<Vec<_>>();
        ...
        for pass in 1..=pass_count {
            ...
            let tab_depth = (depth_mm - tab_height_mm).max(0.0);
            for index in 1..=points.len() {
                ...
                // Split at tab boundaries so segment density cannot shorten a bridge.
                let mut stops = vec![travelled, travelled + length];
                if pass_depth > tab_depth {
                    for &(start, end) in &intervals {
                        for boundary in [start, end] {
                            if boundary > travelled && boundary < travelled + length {
                                stops.push(boundary);
                            }
                        }
                    }
                ...
                for pair in stops.windows(2) {
                    ...
                    let tab = pass_depth > tab_depth
                        && intervals
                            .iter()
                            .any(|&(start, end)| midpoint > start && midpoint < end);
                    if tab != raised {
                        let z = self.surface_z_mm - if tab { tab_depth } else { pass_depth };
                        writeln!(self.source, "G1 Z{} F{}", number(z), number(plunge)).unwrap();
                        raised = tab;
                    }
                    let fraction = ((pair[1] - travelled) / length).clamp(0.0, 1.0);
                    writeln!(
                        self.source,
                        "G1 X{} Y{} F{}",
                        number(from.0 + (to.0 - from.0) * fraction),
                        number(from.1 + (to.1 - from.1) * fraction),
                        number(feed)
                    )
                    .unwrap();
                    motions += 1;
...
fn require_tool(
    ...
    if !allowed.contains(&tool.kind) {
        return Err(PcbError::IncompatibleTool {
            operation,
            tool: tool.name.clone(),
        });
    }
    validate_range("tool.diameterMm", tool.diameter_mm, 0.01, 500.0)?;
    validate_range("tool.cuttingLengthMm", tool.cutting_length_mm, 0.1, 1_000.0)?;
    validate_range("tool.feedMmPerMin", tool.feed_mm_per_min, 1.0, 100_000.0)?;
    validate_range("tool.plungeMmPerMin", tool.plunge_mm_per_min, 1.0, 50_000.0)?;
    validate_range(
        "tool.stepdownMm",
        tool.stepdown_mm,
        0.001,
    ...
    validate_range("tool.stepoverPercent", tool.stepover_percent, 1.0, 95.0)?;
    Ok(())
}
